...
public class PageNavigation {
	...
	public Map<String, Object> getMynavigator() {
		int startPage = (currentPage - 1) / naviSize * naviSize + 1;
		int endPage = startPage + naviSize - 1;
		if(totalPageCount < endPage)
			endPage = totalPageCount;
		Map<String, Object> mynavigator = new HashMap<String, Object>();
		mynavigator.put("startPage", startPage);
		mynavigator.put("endPage", endPage);
		mynavigator.put("startRange", this.startRange ? 1 : (startPage - 1));
		mynavigator.put("endRange", this.endRange ? endPage : (endPage + 1));
		mynavigator.put("totalPageCount", this.totalPageCount);
		mynavigator.put("currentPage", this.currentPage);
		return mynavigator;
	}
